...
#### Особенности  
ul Первое приложение при старте получает параметр – целое число, сколько асинхронных расчетов начать. Все расчеты
идут параллельно.  
ul Передача данных от 1 к 2 идет через Rest WebApi  
ul Передача данных от 2 к 1 идет посредством MessageBus.  
ul Язык C#, среда MS .NET Framework версии от 4.0.  
